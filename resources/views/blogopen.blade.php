...
<head>
    ...
    <script src="https://cdn.tiny.cloud/1/4briqa25g9oadjn92w4pf4e553tnxjoz3vssb2plryk8owbz/tinymce/6/tinymce.min.js"
                            ...
                            @foreach ($comments as $comment)
                                <div class="comment-block ms-2 ms-sm-3 mb-3" data-id="{{ $comment->id }}">
                                    ...
                                    <div class="comment-actions ms-2 my-2">
                                        ...
                                        <form action="" id="form-reply-{{ $comment->id }}"
                                            class="form_reply d-none ms-3">
                                    ...
                                    @if ($comment->replies > 0)
                                        <a href=""
                                            class="btn btn-outline-primary rounded-pill view-replies ms-3 mb-3">{{ $comment->replies == 1 ? $comment->replies . ' reply' : $comment->replies . ' replies' }}</a>
                                    @endif
                ...
                setup: function(editor) {
                    ...
                    editor.on('keydown', function(e) {
                        ...
                        if (e.keyCode === 13 && !e.shiftKey) {
                            ...
                            form.submit();
                        } else if ((e.keyCode >= 33 && e.keyCode <= 40) || e.ctrlKey || e
                            .altKey) {
                            return;
                        } else {
                            setTimeout(function() {
                                var selection = editor.selection.getContent();
                                var anchorTag = editor.dom.getParent(editor.selection
                                    .getNode(), 'a');
                                ...
                                if (anchorTag) {
                                    ...
                                    var newTextNode = editor.getDoc().createTextNode(
                                        contentInside);

                                    // Replace the anchor tag with the new text node
                                    anchorTag.parentNode.replaceChild(newTextNode,
                                        anchorTag);
                        ...
                        if (data.check) {
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (data.liked) {
                                        likeIcon.removeClass('fa-regular').addClass(
                                            'fa-solid');
                                    } else {
                                        likeIcon.removeClass('fa-solid').addClass(
                                            'fa-regular');
                                    }
                                    var contentHtml = `
                        ...
                        success: function(data) {
                            ...
                            if (comments.length > 0) {
                                comments.forEach(function(comment) {
                                    var duplicateElement = $('[data-id="' + comment.id +
                                        '"].comment');
            ...
            $('body').on('click', '.reply-btn', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var contentHtml =
                            `<a href="" class="comment-mention">@${comment.name}</a>&nbsp;`;
                        ...
                        success: function(data) {
                            ...
                            if (replies.length > 0) {
                                replies.forEach(function(reply) {
                                    var duplicateElement = $('[data-id="' + reply.id +
                                        '"].comment');
                                    if (duplicateElement.length <= 0) {
                                        var contentHtml = `
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (viewReplies.hasClass('d-none')) {
                                        viewReplies.removeClass('d-none').addClass(
                                            'd-block');
                                        viewReplies.click();
                                    }
                                    form.reset();
